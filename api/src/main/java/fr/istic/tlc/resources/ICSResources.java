...
public class ICSResources {
    ...
    public EventDTOAndSelectedChoice parseCalendartoAppointments(@PathParam("slug") String slug,
        ...
        final EventDTOAndSelectedChoice result = new EventDTOAndSelectedChoice();
        final List<EventDTO> appointments = new ArrayList<>();
        final List<Long> selectedChoices = new ArrayList<>();
        ...
        final byte[] decodedBytes = Base64.getDecoder().decode(ics);
        ...
        if (!"".equals(decodedString)) {
            ...
            client.start();
            final HttpGet request = new HttpGet(decodedString);
            ...
            // Parse result
            final StringReader sin = new StringReader(responseString);
            final CalendarBuilder builder = new CalendarBuilder();
            final Calendar calendar = builder.build(sin);
            final ComponentList<CalendarComponent> events = calendar.getComponents(Component.VEVENT);
            List<Choice> choices = new ArrayList<Choice>();
			if (p != null) {
				choices = p.getPollChoices();
			}
            // Create Event to draw
            final java.util.Calendar calEnd = java.util.Calendar.getInstance();
            calEnd.setTime(new Date());
            calEnd.add(java.util.Calendar.YEAR, 1);
            final DateTime start = new DateTime(minDate);
            final DateTime end = new DateTime(calEnd.getTime());
            for (CalendarComponent event : events) {

                final Period period = new Period(start, end);
                final PeriodList list = event.calculateRecurrenceSet(period);
                for (Period p1 : list) {
                    if (minDate.before(p1.getStart())) {
                        final EventDTO a = new EventDTO();
