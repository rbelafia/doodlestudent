...
public class UserResource {
    ...
    public ResponseEntity<List<User>> retrieveAllUsers() {
        ...
        final List<User> users = userRepository.findAll(Sort.by("username", Sort.Direction.Ascending)).list();
    ...
    public ResponseEntity<User> updateUser(@PathVariable long idUser, @Valid @RequestBody User user) {
        // On vérifie que l'utilisateur existe
        final User optionalUser = userRepository.findById(idUser);
        if (optionalUser == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        // On met le bon id sur l'utilisateur
        final User ancientUser = optionalUser;
        if (user.getUsername() != null) {
            ancientUser.setUsername(user.getUsername());
        }
        // On update l'utilisateur dans la bdd
        final User updatedUser = userRepository.getEntityManager().merge(ancientUser);
